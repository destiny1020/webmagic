...
public class QuickStarter {
  ...
  public static void main(String[] args) {
    ...
    //Start spider
    OOSpider.create(Site.me(), clazzMap.get(key)).addUrl(urlMap.get(key))
        .addPipeline(new MultiPagePipeline()).addPipeline(new ConsolePipeline()).runAsync();
  ...
  private static String readKey(String key) {
    ...
    for (Map.Entry<String, Class> classEntry : clazzMap.entrySet()) {
      System.out.println(classEntry.getKey() + "\t" + classEntry.getValue() + "\t" + urlMap
          .get(classEntry.getKey()));
    }
    while (key == null) {
      key = stdin.nextLine();
