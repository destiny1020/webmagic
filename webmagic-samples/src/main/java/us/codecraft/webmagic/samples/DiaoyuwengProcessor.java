...
/**
 * @author code4crafter@gmail.com <br> Date: 13-4-21 Time: 下午8:08
 */
public class DiaoyuwengProcessor implements PageProcessor {
  ...
  public void process(Page page) {
    List<String> requests = page.getHtml().links().regex(
    ...
    requests = page.getHtml().links().regex("(http://www\\.diaoyuweng\\.com/thread-\\d+-1-1.html)")
        .all();
    page.addTargetRequests(requests);
    if (page.getUrl().toString().contains("thread")) {
      ...
      page.putField("id", new PlainText(
          "1000" + page.getUrl().regex("http://www\\.diaoyuweng\\.com/thread-(\\d+)-1-1.html")
              .toString()));
    }
  }
  ...
  public static void main(String[] args) {
    Spider.create(new DiaoyuwengProcessor()).addUrl(
