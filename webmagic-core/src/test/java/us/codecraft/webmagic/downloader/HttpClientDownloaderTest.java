...
public class HttpClientDownloaderTest {
      ...
      private String getCharsetByUrl(String url) {
        ...
        try {
          httpResponse = httpClient.execute(
              new HttpUriRequestConverter().convert(requestGBK, site, null).getHttpUriRequest());
        } catch (IOException e) {
          e.printStackTrace();
        }
        String charset = null;
        try {
          ...
          charset = CharsetUtils
              .detectCharset(httpResponse.getEntity().getContentType().getValue(), contentBytes);
        } catch (IOException e) {
          e.printStackTrace();
        }
        return charset;
  ...
  public void test_selectRequestMethod() throws Exception {
    ...
    server.request(and(by(method("HEAD")), eq(query("q"), "webmagic")))
        .response(header("method", "head"));
    ...
    Runner.running(server, new Runnable() {
      ...
      public void run() throws Exception {
        ...
        HttpUriRequest httpUriRequest = httpUriRequestConverter.convert(request, site, null)
            .getHttpUriRequest();
        assertThat(
            EntityUtils.toString(HttpClients.custom().build().execute(httpUriRequest).getEntity()))
            .isEqualTo("get");
        ...
        httpUriRequest = httpUriRequestConverter.convert(request, site, null).getHttpUriRequest();
        assertThat(HttpClients.custom().build().execute(httpUriRequest).getFirstHeader("method")
            .getValue()).isEqualTo("head");
  ...
  public void test_disableCookieManagement() throws Exception {
    ...
    Runner.running(server, new Runnable() {
      ...
      public void run() throws Exception {
        ...
        request.setUrl("http://127.0.0.1:13423");
        request.addCookie("cookie", "cookie-webmagic");
        Page page = httpClientDownloader
            .download(request, Site.me().setDisableCookieManagement(true).toTask());
        assertThat(page.getRawText()).isEqualTo("ok");
      }
    });
  }
  ...
  public void test_set_site_header() throws Exception {
    ...
    Runner.running(server, new Runnable() {
      ...
      public void run() throws Exception {
        ...
        request.setUrl("http://127.0.0.1:13423");
        Page page = httpClientDownloader
            .download(request, Site.me().addHeader("header", "header-webmagic").toTask());
        assertThat(page.getRawText()).isEqualTo("ok");
      }
    });
  }
    ...
    Runner.running(server, new Runnable() {
      ...
      public void run() throws Exception {
        ...
        httpClientDownloader.setProxyProvider(
            SimpleProxyProvider.from(new Proxy("127.0.0.1", 13423, "username", "password")));
        Request request = new Request();
